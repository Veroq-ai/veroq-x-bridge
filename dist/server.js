...
// ── API Helpers ──
async function veroqApi(method, path, params, body) {
...
function analyzeSentiment(text) {
    ...
    if (total === 0)
        return { score: 0, label: "neutral", positive_signals: [], negative_signals: [] };
    const score = (pos.length - neg.length) / total;
    const label = pos.length >= 2 && neg.length >= 2 ? "mixed" :
        score > 0.2 ? "positive" :
...
function aggregateSentiment(results) {
    ...
    for (const r of results) {
        dist[r.label]++;
        totalScore += r.score;
        for (const w of r.positive_signals)
            allPos[w] = (allPos[w] || 0) + 1;
        for (const w of r.negative_signals)
            allNeg[w] = (allNeg[w] || 0) + 1;
    }
    const avg = Math.round((totalScore / results.length) * 100) / 100;
    const label = avg > 0.15 ? "positive" :
        avg < -0.15 ? "negative" :
...
async function searchXPosts(query, maxResults = 10) {
...
server.tool("x_verify_post", `Verify factual claims in an X (Twitter) post using VeroQ Shield.
...
COST: 1 X API call + 1 VeroQ Shield call.`, {
    ...
    if (!postId)
        return text("Invalid post URL or ID.");
    const xPost = await getXPost(postId);
    if (!xPost)
        return text(`Post ${postId} not found or inaccessible.`);
    // Verify with Shield
    ...
    for (const c of claims) {
        ...
        lines.push(`    Verdict: ${c.verdict} (${Math.round(c.confidence * 100)}%)`);
        if (c.correction)
...
server.tool("x_topic_sentiment", `Get real-time social sentiment for a topic from X posts, combined with VeroQ entity sentiment if available.
...
COST: 1 X API search + 1 optional VeroQ entity call.`, {
    ...
    if (posts.length === 0)
        return text(`No recent X posts found for "${topic}".`);
    ...
    if (include_veroq_sentiment) {
        try {
            ...
            if (entity.status === "ok") {
                ...
                const trend = entity.sentiment_trend;
...
server.tool("x_verify_trending", `Find trending posts about a topic on X and fact-check the most-shared claims using VeroQ Shield.
...
COST: 1 X API search + N VeroQ Shield calls (one per verified post).`, {
    ...
    const verifications = await Promise.all(topPosts.map(async (post) => {
    ...
    for (const { post, shield } of verifications) {
        ...
        if (!shield) {
            ...
            if (claims.length === 0) {
                ...
                for (const c of claims) {
                    const icon = c.verdict === "supported" ? "+" : c.verdict === "contradicted" ? "x" : "?";
                    lines.push(`  [${icon}] ${c.text} — ${c.verdict} (${Math.round(c.confidence * 100)}%)`);
                    if (c.correction)
                        lines.push(`      Correction: ${c.correction}`);
                }
            }
...
server.tool("x_entity_pulse", `Full intelligence on an entity: X social sentiment + VeroQ news sentiment + latest briefs + trending co-mentions.
...
COST: 1 X API search + 3 VeroQ API calls.`, {
    ...
    const lines = [`## Entity Pulse: ${entity}`, ""];
    // X Social Sentiment
    if (xPosts.length > 0) {
        ...
        if (agg.top_signals.positive.length)
            lines.push(`**Bullish:** ${agg.top_signals.positive.join(", ")}`);
        if (agg.top_signals.negative.length)
            lines.push(`**Bearish:** ${agg.top_signals.negative.join(", ")}`);
        // Most-liked post
    ...
    if (veroqSentiment?.status === "ok") {
        const overall = veroqSentiment.overall;
        const trend = veroqSentiment.sentiment_trend;
    ...
    if (veroqBriefs?.status === "ok") {
        const briefs = (veroqBriefs.briefs || []);
        if (briefs.length > 0) {
            lines.push("### Latest Verified News");
            for (const b of briefs.slice(0, 5)) {
                const conf = b.confidence_score ? ` (${Math.round(b.confidence_score * 100)}% confidence)` : "";
    ...
    if (veroqRelationships?.status === "ok") {
        const rels = (veroqRelationships.relationships || []);
        if (rels.length > 0) {
            lines.push("### Related Entities");
            for (const r of rels.slice(0, 8)) {
                const sent = r.avg_sentiment;
...
server.tool("x_search_verified", `Search X posts matching a query and verify the most-engaged post's claims via VeroQ Shield.
...
COST: 1 X API search + 1 VeroQ Shield call.`, {
    ...
    if (posts.length === 0)
        return text(`No posts found for "${query}".`);
    const sorted = [...posts].sort((a, b) => (b.public_metrics?.like_count || 0) - (a.public_metrics?.like_count || 0));
    const lines = [
        `## X Search: "${query}"`,
